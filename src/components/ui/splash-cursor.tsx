...
function SplashCursor({
  ...
  useEffect(() => {
    ...
    function generateColor() {
      // Generate blue tones matching the theme (blue-400 to blue-600)
      // Hue for blue is around 0.6 (210 degrees)
      const hue = 0.55 + Math.random() * 0.1; // 0.55-0.65 for blue range
      const saturation = 0.7 + Math.random() * 0.3; // 0.7-1.0 for vibrant blues
      const value = 0.5 + Math.random() * 0.5; // 0.5-1.0 for brightness
      let c = HSVtoRGB(hue, saturation, value);
      // Increase intensity for more visible effect
      c.r *= 0.25;
      c.g *= 0.25;
      c.b *= 0.25;
      return c;
